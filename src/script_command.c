...
script_command_status script_set_detector_val(script_session *s, int val, int argc, char *const *argv) {
    ...
    switch(val) {
        ...
        case 'r': /* resolution */
            ...
            unit_type = JIBAL_UNIT_TYPE_ANY;
            break;
        case 'R': /* resolution slope */
            value_double = calibration_get_param_ref(det->calibration, CALIBRATION_PARAM_RESOLUTION_SLOPE);
            unit_type = JIBAL_UNIT_TYPE_ANY;
            break;
        case 's': /* solid */
...
script_command_status script_set_detector_calibration_val(script_session *s, int val, int argc, char *const *argv) {
    ...
    switch(val) {
        ...
        case 'r': /* resolution */
            ...
            return 1;
        case 'R': /* resolution slope */
            if(calibration_set_param(c, CALIBRATION_PARAM_RESOLUTION_SLOPE, value_dbl)) {
                jabs_message(MSG_ERROR, "Can not set calibration resolution slope.\n");
                return SCRIPT_COMMAND_FAILURE;
            }
            return 1;
        default:
...
script_command *script_commands_create(script_session *s) {
    ...
    script_command_list_add_command(&c_calibration->subcommands, script_command_new("rslope", "Set the resolution slope.", 'R', 0, NULL));
    ...
    script_command_list_add_command(&c_detector->subcommands, script_command_new("rslope", "Set detector resolution slope.", 'R', 0, NULL));
    ...
    const jibal_config_var vars[] = {
        ...
        {JIBAL_CONFIG_VAR_INT,    "ds_steps_azi",                  0,     0,                               &sim->params->ds_steps_azi,                  NULL, "Dual scattering azimuthal steps (0 = default)"},
        {JIBAL_CONFIG_VAR_INT,    "ds_steps_polar",                0,     0,                               &sim->params->ds_steps_polar,                NULL, "Dual scattering polar steps (0 = default)"},
        {JIBAL_CONFIG_VAR_INT,    "ds_steps_polar_substeps",       0,     0,                               &sim->params->ds_steps_polar_substeps,       NULL, "Dual scattering polar substeps (0 = default)"},
